...
function Header({ className, children, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) { return <thead className={cn("bg-bg-weak-50 text-label-xs text-text-sub-600", className)} {...props}>{children}</thead>; }
...
function Head({ className, children, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) { return <th className={cn("h-11 px-4 py-2 font-medium", className)} {...props}>{children}</th>; }
function Cell({ className, children, ...props }: React.TdHTMLAttributes<HTMLTableCellElement>) { return <td className={cn("px-4 py-3.5 align-middle", className)} {...props}>{children}</td>; }
